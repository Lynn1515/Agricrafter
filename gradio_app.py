...
from data_process import center_crop_resize2
...
def dynamicrafter_demo(result_dir='./tmp/', res=1024):
    ...
    with gr.Blocks(analytics_enabled=False, css=css) as dynamicrafter_iface:
        gr.Markdown("<div align='center'> <h1> test DynamiCrafter </span> </h1>")
            ...
            with gr.Tab(label='Image2Video_320x512'):
                with gr.Column():
                    with gr.Row():
                        with gr.Column():
                            with gr.Row():
                                i2v_input_image = gr.Image(label="Input Image",elem_id="input_img", type="numpy")
                                #i2v_input_image = center_crop_resize2(i2v_input_image)
                            with gr.Row():
                                i2v_input_text = gr.Text(label='Prompts')
                            with gr.Row():
                                i2v_eta = gr.Slider(minimum=0.0, maximum=1.0, step=0.1, label='ETA', value=1.0, elem_id="i2v_eta")
                ...
                def process_and_generate(image, text, steps, cfg_scale, eta, motion, seed):
                    # 对图像进行裁剪
                    cropped_image = center_crop_resize2(image)
                    # 生成视频
                    return image2video.get_image(cropped_image, text, steps, cfg_scale, eta, motion, seed)
                i2v_end_btn.click(inputs=[i2v_input_image, i2v_input_text, i2v_steps, i2v_cfg_scale, i2v_eta, i2v_motion, i2v_seed],
                                outputs=[i2v_output_video],
                                fn = process_and_generate#image2video.get_image
                )
        elif res == 256:
